...
class Plotter:
    ...
    def _plot_group(
        self, ax, group, x_axis_data_type, y_axis_data_type, linestyle_colors
    ):
        for entry, (linestyle, color) in zip(group, linestyle_colors):
            if self.plot_type == "line":
                ax.plot(
                    entry.data[x_axis_data_type],
                    entry.data[y_axis_data_type],
                    label=entry.label,
                    linestyle=linestyle,
                    color=color,
                )
    ...
    def create_plot(self, groups, x_axis_data_type, y_axis_data_type):
        ...
        for i, group in enumerate(groups):
            self._plot_group(
                axs[i], group, x_axis_data_type, y_axis_data_type, linestyle_colors
            )
