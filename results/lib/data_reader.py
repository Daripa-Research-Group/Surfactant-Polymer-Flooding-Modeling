...
class DataReader:
    ...
    def get_sheet_data(self, sheet, field_name, polymer_type):
        ...
        if (
            timestamp_col >= max_columns
            or coc_col >= max_columns
            or mfw_col >= max_columns
        ):
        ...
        while cur_row < max_rows:
            if (
                pd.isna(file.iloc[cur_row, timestamp_col])
                or pd.isna(file.iloc[cur_row, coc_col])
                or pd.isna(file.iloc[cur_row, mfw_col])
            ):
                break
