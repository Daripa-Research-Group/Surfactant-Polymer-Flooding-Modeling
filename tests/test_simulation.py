...
def initializing_simulation():
    ...
    polymer_object = Polymer(
        PolymerList.Xanthane,
        0.001,
        PolymerList.Xanthane.e_coeff,
        PolymerList.Xanthane.n_coeff,
    )

    # initializing surfactant object
    lambda_IFT_equation = lambda GG: 10.001 / (GG + 1) - 0.001
    deriv_IFT_equ = lambda GG: (-10.001) / ((GG + 1) ** 2)
    surfactant_object = Surfactant(
        SurfactantList.Alkyl_Ether_Sulfate, 0, lambda_IFT_equation, deriv_IFT_equ
    )
...
def test_compute_viscosity():  # tested
    ...
    [x, y] = np.meshgrid(
        np.arange(
            test_sim_object.mesh.left,
            test_sim_object.mesh.right + test_sim_object.mesh.dx,
            test_sim_object.mesh.dx,
        ),
        np.arange(
            test_sim_object.mesh.bottom,
            test_sim_object.mesh.top + test_sim_object.mesh.dy,
            test_sim_object.mesh.dy,
        ),
    )
    ...
    print(
        "Polymer Concentration (vector form):",
        test_sim_object.polymer.vec_concentration,
    )
    c0_array = test_sim_object.polymer.initial_concentration * np.ones(
        (sog + 1, sog + 1)
    )
    print("c0array:", c0_array)
    test_sim_object.compvis(
        u, v, x, y, beta1, c0_array
    )  # Issue with calculating c0array
...
def test_divergence():
    ...
    np.testing.assert_allclose(
        computed_div, expected_div, atol=1e-1
