...
class Simulation:
    ...
    def derivative_sigma(self):
        return self.surfactant.derivative_IFT_conc_equ(
            np.array(self.surfactant.vec_concentration)
        )
    ...
    def execute_simulation(self):
        ...
        bool_rectilinear_homogenous = (
            self.permeability_flg == PermeabilityType.Homogenous
            and self.resevoir_geometry == ResevoirGeometry.Rectilinear
        )
        bool_rectilinear_heterogenous = (
            self.permeability_flg == PermeabilityType.Heterogenous
            and self.resevoir_geometry == ResevoirGeometry.Rectilinear
        )
        bool_quarterfivespot_heterogenous = (
            self.permeability_flg == PermeabilityType.Heterogenous
            and self.resevoir_geometry == ResevoirGeometry.Quarter_Five_Spot
        )
        ...
        if bool_rectilinear_homogenous or bool_rectilinear_heterogenous:
            ...
            f[self.mesh.n, self.mesh.m] = (
                -1 * mag_source_flow
            )  # intensity of production well
        ...
        if Kinfo is not None:
            ...
            raise SimulationCalcInputException(
                "SimulationError: 'KK' and 'Kmax' variables not instantiated..."
            )

        # Initializing the water saturation, polymer concentration, and surfactant concentration matrix
        c0_array = self.polymer.initial_concentration * np.ones(
            (
        ...
        beta_1 = 15000
        self.polymer.viscosity_scalar = viscosity_water * (
            1 * beta_1 * self.polymer.initial_concentration
        )
    ...
    def get_phi_value(self):
        ...
        try:
            ...
            if isinstance(self.mesh, Box):
                ...
                raise SimulationCalcInputException(
                    "SimulationError | get_phi_value | mesh values not provided"
                )
    ...
    def initial_concentration_matrix(self):
        ...
        try:
            if (
                self.polymer is not None
                and self.surfactant is not None
                and self.init_water_saturation_scalar is not None
            ):
                ...
                raise SimulationCalcInputException(
                    "SimulationError: Did not provide intial water saturation or Polymer and/or Surfactant were not intialized. Please try again..."
                )
            ...
            if self.phi is not None:
                ...
                raise SimulationCalcInputException(
                    "SimulationError: phi value not calculated!"
                )
            ...
            return [
                ...
                self.polymer.vec_concentration,
    ...
    def compvis(self, U, V, X, Y, beta1, c0_array):
        ...
        try:
            if self.polymer is not None and self.surfactant is not None:
                ...
                raise SimulationCalcInputException(
                    "SimulationError: Surfactant and/or Polymer Not Initialized"
                )
            ...
            if self.mdl_id == ModelType.No_Shear_Thinning:
                ...
                if polymer_obj.name == PolymerList.Xanthane.Id:
                    ...
                    raise SimulationCalcInputException(
                        "SimulationError: Polymer Not Part of 'PolymerTypes' Enumerations List"
                    )
                ...
                pi_D = np.abs(
                    -0.25 * ((dList[0] + dList[1]) ** 2) + (dList[2] * dList[3])
                )
                    ...
                    for jj in range(m):
                        if polymer_obj.vec_concentration[ii, jj] > 0:
                            ...
                            if gamma_dot[ii, jj] != 0:
                                polymer_obj.vec_concentration[ii, jj] = e_power_value[
                                    ii, jj
                                ] * (gamma_dot[ii, jj] ** (n_power_value[ii, jj] - 1))
                                self.aqueous_viscosity[ii, jj] = e_vector[ii, jj] * (
                                    gamma_dot[ii, jj] ** (n_vector[ii, jj] - 1)
                                )
                ...
                raise SimulationCalcInputException(
                    "SimulationError: Model Type not part of 'ModelType' Enumerations list"
                )
    ...
    def compmob(self, sor, swr, flag):
        ...
        if (
            self.is_surfactant == 0 and self.water_saturation is not None
        ):  # (without surfactant)
            ...
            if flag == 0:  # calculating mobility in oleic phase
                ...
                raise SimulationCalcInputException(
                    "SimulationError: Flag unknown. Flag can only be a 0 (for oleic mobility calculation) and 1 (for aqueous mobility calculation). Please try again..."
                )
        elif (
            self.is_surfactant == 1 and self.water_saturation is not None
        ):  # (with surfactant)
            ...
            raise SimulationCalcInputException(
                "SimulationError: Need proper statement of is_surfactant (0 for Polymer Flooding Simulation & 1 for Surfactant-Polymer Flooding Simulation). Please try again..."
            )
    ...
    def setTri(self):
        ...
        for j in range(0, self.mesh.m):
            for k in range(0, self.mesh.n):
                ...
                l = {"x": [x1, x2, x3], "y": [y1, y2, y3]}

                u = {"x": [x4, x3, x2], "y": [y4, y3, y2]}
    ...
    def saturation_equ_solver(self, dt, u, v):
        ...
        if self.water_saturation is not None:
            ...
            raise SimulationCalcInputException(
                "SimulationError: Water Saturation matrix not initialized..."
            )
        ...
        if KK_info is not None:
            ...
            raise SimulationCalcInputException(
                "SimulationError: KK and Kmax not calculated properly..."
            )
        ...
        pc_g = (pc / self.sigma) * sigma_g + pc_s
        f_c = (lambda_o * lambda_a * miuo) / (
            (lambda_total**2) * self.aqueous_viscosity
        )
        f_g = (kra_g * lambda_o) / ((lambda_total**2) * self.aqueous_viscosity) - (
            kro_g * lambda_a
        ) / ((lambda_total**2) * miuo)
                    ...
                    if idx == 1:
                        if i == 0:  # first/left column
                            DD[i] = (
                                ...
                                * self.surfactant.vec_concentration[cnt][i]
                                - (D_g[cnt][i] + D_g[cnt][i + 1])
                                / (dx**2)
                                * self.surfactant.vec_concentration[cnt][i + 1]
                                - (D_g[cnt][i] + D_g[cnt + 2][i])
                                / (dy**2)
                                * self.surfactant.vec_concentration[cnt + 2][i]
                            )
                            ...
                            DD[i] = (
                                ...
                                * self.surfactant.vec_concentration[cnt][i - 1]
                                ...
                                / (dy**2)
                                * self.surfactant.vec_concentration[cnt + 1][i]
                            )
                            ...
                            DD[i] = (
                                ...
                                * (
                                    u[cnt][i]
                                    * (
                                        self.polymer.vec_concentration[cnt][i + 1]
                                        - self.polymer.vec_concentration[cnt][i - 1]
                                    )
                                    / (2 * dx)
                                )
                        ...
                        if i == 0:
                            DD[i] = (
                                ...
                                * (
                                    v[cnt][i]
                                    * (
                                        self.polymer.vec_concentration[cnt + 1][i]
                                        - self.polymer.vec_concentration[cnt][i]
                                    )
                                    / (2 * dy)
                                )
                                - f_g[cnt][i]
                        ...
                        if idx == 1:
                            ...
                            if i == 0:
                                ...
                                BB[j][i] = (
                                    ...
                                    / (Qnew[cnt][i] * self.surfactant.concentration)
                                )
                                BB[j][i - 1] = 2 * F[cnt][i] / (dx**2)
                            else:
                                DD[i] = Gmod[cnt][i] / dt[cnt][i]
    ...
    def eval_Xsurf_neumann(
        self, flag, x, y, s, snew, g, f, f_s, D, pc_s, pc_g, u, v, dt
    ):
        x_jump = None
        y_jump = None
        if flag == 1:
    ...
    def KK_def(self, x, y):
        ...
        if (
            ...
            KK = Kmax * (
                ...
                * (1 - 10 ^ (-7))
                * (
                    np.sin(6 * np.pi * np.cos(x)) * np.cos(4 * np.pi * np.sin(3 * y))
                    - 1
                )
                + 1
            )
